...
- **Declaring an operation, when recording is on.** With `signals: on` in the store — off unless the person set it — put a short header at the top of artifacts *this discipline authors*: a written map, a stated path, a check committed to paper, a settled domain-how. `howdo_operation` says what the operation was and `howdo_stage` which stage it belonged to; at Check add `howdo_expects` (what this commits to), at Look add `howdo_held: yes|no`, and where they apply `howdo_domain` and `howdo_shape: instance-first|principle-first`. A hook records that the artefact landed, which is the half you do not author: your declaration is a claim, the file existing is the check on it, and a `held: yes` on something rewritten minutes later is a measurable overconfidence rather than a success. Declare what you actually committed to, not what would look good.
