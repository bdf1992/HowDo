...
|---|---|
| untested artifact installed where a host will load it | refused; the emitter's guard applies to every destination |
...
| two sessions racing to settle one store | writers serialize on an exclusive lock; the loser re-validates on current bytes and its own state guard refuses the stale write |
| settlement torn mid-write by process death | writes go through a temp file and atomic replace; a reader sees the prior lineage or the new one, never a fragment |
| stale author silently erasing a landed settlement | refused; the prior bytes survive byte-for-byte |
| lock abandoned by a dead writer | broken after a stale threshold instead of deadlocking future settlement |
