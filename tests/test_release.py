...
class ReleaseContractTests(unittest.TestCase):
    def test_release_versions_are_aligned(self):
        ...
        # Fifth place, and the only one a plugin host reads. It is tracked
        # rather than derived because a marketplace clones the plugin root and
        # reads the manifest as committed, with no build step in between.
        manifest = (ROOT / "plugin" / ".claude-plugin" / "plugin.json").read_text(encoding="utf-8")
